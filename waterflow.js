...
var waterflow = (function() {
	 ...
	 * @param  {[object]} _option [the same as before]
	 * @return {[function]} water
	 */
	function responsive(_option) {
		var deviceWidth = document.documentElement.offsetWidth || document.body.offsetWidth,
			itemWidth = _option.width;

		var col = Math.floor(deviceWidth / itemWidth);

		// calculate the true col of the waterflow according to the formula
		// that compare (the item's width +  the margin between them) with (deviceWidth)
		_option.column = (col * itemWidth + (col - 1) * _option.horizontalMargin) < deviceWidth ? col : col - 1;
		water(_option);
	}

	/**
	 * [set the layout of waterflow]
	 * @param  {[object]} _option [the same as before]
	 * @return {[undefined]}
	 */
